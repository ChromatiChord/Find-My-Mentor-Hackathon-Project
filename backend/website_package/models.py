...
ment_post_args.add_argument('email', type=str, help='Email is not provided', required=True)
ment_post_args.add_argument('phone', type=str, help="Mentor did not provide phone number")
...
ment_post_args.add_argument('contacts', type=str, help='Mentor did not provide additional contact details')
...
class StudModel(db.Model):
  ...
  subject = db.Column(db.String(100), nullable=False)
  level =  db.Column(db.String(100), nullable=False)
  mentor_id = db.Column(db.Integer, db.ForeignKey('mentors.email'), nullable=True)
  def __repr__(self): 
    return f'Student(user name = {user_name}, first name = {first_name}, last_name = {last_name}, email = {email},phone = {phone}, subject = {subject}, level = {level}'
...
stud_post_args.add_argument('phone', type=int, help='Student did not provide a phone address')
